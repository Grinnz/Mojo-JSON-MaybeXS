...
before Mojolicious so the new functions will be properly exported.

=head1 CAVEATS

L<JSON::MaybeXS> may load different modules depending on what is available, and
these modules have slightly different behavior from each other and from
L<Mojo::JSON>. As of this writing, the author has found the following
incompatibilities:
