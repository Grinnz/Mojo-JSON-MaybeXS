...
Mojo::JSON does not either; in most cases, where Mojo::JSON would stringify a
value, JSON::MaybeXS with these settings will encode it to C<null>. See below
for more specifics.
...
L<Mojo::JSON> and occasionally from each other. As of this writing, the author
has found the following incompatibilities:
